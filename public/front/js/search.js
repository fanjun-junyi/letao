/*
* @Author: fanjun-junyi
* @Date:   2017-11-13 19:34:18
* @Last Modified by:   Administrator
* @Last Modified time: 2017-11-13 20:47:46
*/

$(function(){
   //1. 先获取到localStorage的记录,
   //如果获取时，发现没有这条记录，给一个默认值，就是空数组
   function getHistory(){
   ...
   function render(){
      ...
      var arr = getHistory();
      //使用模板渲染出来
      var html = template("searchTmp", {arr: arr});
   ...
   $('.lt_search button').on('click',function(){
      //清除两边的空格
      var keyWords = $('.lt_search input').val().trim();
      $('.lt_search input').val("");
      ...
      var index = arr.indexOf(keyWords);//获取key在arr中第一次出现的索引
      if(index != -1){
        //存在，删除
        arr.splice(index,1);
      }
